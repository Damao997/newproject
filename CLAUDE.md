...
三套体系（2026-08 科目表重构，依据《科目表重构.xlsx》）：

| 类型 | 格式 | 数量与层级 |
|------|------|------|
| 经营科目 | `PL` + 级联数字（如 `PL02` / `PL0201` / `PL02010101`） | 138 项，4 级层级（level 0-3），段位 01-08（回款/收入/成本/毛利/费用/经营成果/毛利率/经营指标） |
| 静态科目 | `BS` + 级联数字（如 `BS01` / `BS0103`） | 32 项，3 级层级（level 0-2），段位 01-04（总资产/总负债/权益净资产/静态指标） |
| 现金流科目 | `CF` + 级联数字（如 `CF01` / `CF0101`） | 31 项，3 级层级（level 0-2），段位 01-03（经营/投资/筹资活动） |

编码规则：level0 = 前缀 + 段位表登记段位（`prisma/seed-data/subject-trees.ts` 的 `SUBJECT_SEGMENT_MAP`）；子级 = 父码 + 2 位序号。

现金流预算口径：年度预算模板（budget）含经营科目与现金流流入/流出层 6 科目（`CF0101` 等），净额类（`CF01`/`CF02`/`CF03`）与自由现金流（`CF04`）预算由公式层按流入-流出推导，维度码 `CF_BUDGET_AMOUNT`；无预算时关键指标表现金流板块预算列保持 null（前端「—」）。

### 指标编码

| data_type | 编码规则 | 示例 |
|-----------|----------|------|
| data / display | `metric.code = account_subject.code` | `PL02010101`（燃气具-灶具收入） |
| calc（计算类） | 与科目同码，公式存 `metric.formula` | `PL04`（壹品慧毛利 = 收入-成本） |
