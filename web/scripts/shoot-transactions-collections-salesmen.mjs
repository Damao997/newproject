// 只截销售员催收看板一个页面的 React + Design 双截图（用于本地验证高保真度）
// - React: 走 http://127.0.0.1:5173/transactions/collections/salesmen（Vite dev server）
// - Design: 走 http://127.0.0.1:8001/pages/transactions-collections-salesmen.html（design static server）
// 用法：node scripts/shoot-transactions-collections-salesmen.mjs
// 产物：audit/screenshots/{design,react}/transactions-collections-salesmen-{desktop,mobile}.png
...
const SLUG = 'transactions-collections-salesmen'
...
async function installApiMock(ctx) {
  await ctx.route('**/api/v1/**', async (route) => {
    ...
    } else if (url.includes('/salesmen') || url.includes('/salesman')) {
      body = { code: 0, data: { items: [], total: 0 }, message: 'ok' }
    } else if (url.includes('/cross-table') || url.includes('/browse')) {
      body = { code: 0, data: { companies: [], rows: [] }, message: 'ok' }
    } else if (url.includes('/transactions/periods')) {
      body = { code: 0, data: ['2026-01','2026-02','2026-03','2026-04','2026-05','2026-06','2026-07','2026-08'], message: 'ok' }
    } else if (url.includes('/collections')) {
      body = { code: 0, data: { items: [], total: 0, stats: {} }, message: 'ok' }
...
async function waitForReactReady(page) {
  ...
  await page
    ...
    .catch(() => {})
  await page.waitForSelector('text=销售员催收看板', { timeout: 30000 }).catch(() => {})
  await page.waitForSelector('text=销售员业绩卡片', { timeout: 30000 }).catch(() => {})
  await page.waitForSelector('text=王芳', { timeout: 30000 }).catch(() => {})
...
async function main() {
  ...
  // ─── Design 截图 ───
  {
    ...
    for (const vp of [
      ...
      await page
        .goto(`${DESIGN_BASE}/transactions-collections-salesmen.html`, { waitUntil: 'networkidle', timeout: 30000 })
        .catch(() => {})
  ...
  await page
    .goto(`${REACT_BASE}/transactions/collections/salesmen`, { waitUntil: 'domcontentloaded', timeout: 30000 })
    .catch(() => {})
