// 只截运营费用分析（/dashboard/analysis/expense）这一个页面的 React / 设计稿对比图
...
/**
 * 给所有 /api/v1/* 响应打补丁：让前端走"业务正常 + 空数据/mock"分支，
 * 同时保持 isAuthenticated=true，MainLayout 不会强制跳 /login。
 * 关键：运营费用分析目前是纯静态 mock（ExpenseContent 不依赖任何接口），
 * 但仍需拦截刷新 token / 主体维度等公共接口，避免 401 链路触发。
 */
async function installApiMock(ctx) {
  await ctx.route('**/api/v1/**', async (route) => {
    ...
    if (url.includes('/dashboard/overview')) {
      ...
      body = {
        code: 0,
        data: { periods: ['2026-08'], fiscalYears: ['2026'], fiscalStartMonth: 1 },
        message: 'ok',
      }
      ...
      body = {
        code: 0,
        data: { accessToken: 'audit-token', refreshToken: 'audit-refresh', user: AUTH_SEED.state.user },
        message: 'ok',
      }
...
async function waitForPageReady(page) {
  ...
  await page
    ...
    .catch(() => {})
  // 等待费用结构饼图标题渲染（ExpenseContent 唯一标识）
  await page
    .waitForSelector('text=费用结构占比', { timeout: 30000 })
    .catch(() => {})
...
async function main() {
  ...
  await reactPage
    .goto(`${REACT_BASE}/dashboard/analysis/expense`, { waitUntil: 'domcontentloaded', timeout: 30000 })
    .catch(() => {})
  await waitForPageReady(reactPage)
  const reactDesktopPath = join(AUDIT_DIR, 'react', 'expense-desktop.png')
  ...
  console.log('Target desktop ≥ 100 KB')

  if (reactDesktopBytes < 100 * 1024) {
    console.warn('[shoot] WARN: desktop 截图 < 100KB，请检查渲染是否完整')
  }
}
