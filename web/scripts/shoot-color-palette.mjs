// 01 / color-palette: 截 React (/_design/antd-style) + Design (pages/color-palette.html) 全页
// 目标：React desktop bytes >= Design desktop bytes（视觉信息密度对齐设计稿）
// 用法：node scripts/shoot-color-palette.mjs
...
const AUTH_SEED = {
  state: {
    user: {
      ...
      permissions: ['dashboard:view','dashboard:export','indicators:view','indicators:export','transactions:view','transactions:import','transactions:create','transactions:update','transactions:export','inventory:view','reports:view','data:browse:view','data:reclassify','data:export','data:import','tools:view','admin:users:view','admin:roles:view'],
      dataScope: 'all', status: 'active', createdAt: new Date().toISOString(), updatedAt: new Date().toISOString(),
    },
...
async function waitForReactReady(page) {
  ...
  await page.waitForSelector('text=antd-style', { timeout: 30000 }).catch(() => {})
  await page.waitForSelector('text=Color Palette', { timeout: 30000 }).catch(() => {})
...
async function main() {
...

  {
    ...
    for (const vp of [{ name: 'desktop', width: 1440, height: 900 }, { name: 'mobile', width: 390, height: 844 }]) {
      ...
      await page.goto(`${DESIGN_BASE}/color-palette.html`, { waitUntil: 'networkidle', timeout: 30000 }).catch(() => {})
  ...
  console.log(`===== 01 Color Palette / ${SLUG} =====`)
