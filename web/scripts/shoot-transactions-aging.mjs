// 只截账龄分析 / transactions-aging 一个页面的 React + Design 双截图（用于本地验证高保真度）
// - React: 走 http://127.0.0.1:5173/transactions/aging（Vite dev server）
// - Design: 走 http://127.0.0.1:8001/pages/transactions-aging.html（design static server）
// 用法：node scripts/shoot-transactions-aging.mjs
// 产物：audit/screenshots/{design,react}/transactions-aging-{desktop,mobile}.png
...
async function installApiMock(ctx) {
  await ctx.route('**/api/v1/**', async (route) => {
    ...
    } else if (url.includes('/transactions/aging') || url.includes('/transactions/overview')) {
      body = { code: 0, data: { kpi: {}, rows: [], summary: {} }, message: 'ok' }
...
async function waitForReactReady(page) {
  ...
  await page
    ...
    .catch(() => {})
  await page.waitForSelector('text=账龄分析', { timeout: 30000 }).catch(() => {})
  await page.waitForSelector('text=账龄段占比', { timeout: 30000 }).catch(() => {})
  await page.waitForSelector('text=账龄明细表', { timeout: 30000 }).catch(() => {})
  await page.waitForSelector('text=杭州绿城物业管理有限公司', { timeout: 30000 }).catch(() => {})
...
async function main() {
  ...
  // ─── Design 截图 ───
  {
    ...
    for (const vp of [
      ...
      await page
        .goto(`${DESIGN_BASE}/transactions-aging.html`, { waitUntil: 'networkidle', timeout: 30000 })
        .catch(() => {})
  ...
  await page
    .goto(`${REACT_BASE}/transactions/aging`, { waitUntil: 'domcontentloaded', timeout: 30000 })
    .catch(() => {})
