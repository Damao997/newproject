// 只截角色管理 / admin-roles 一个页面的 React + Design 双截图（用于本地验证高保真度）
// - React: 走 http://127.0.0.1:5173/admin/roles（Vite dev server）
// - Design: 走 http://127.0.0.1:8001/pages/admin-roles.html（design static server）
// 用法：node scripts/shoot-admin-roles.mjs
// 产物：audit/screenshots/{design,react}/admin-roles-{desktop,mobile}.png
...
async function installApiMock(ctx) {
  await ctx.route('**/api/v1/**', async (route) => {
    ...
    } else if (url.includes('/admin/roles') || url.includes('/roles')) {
      body = {
        code: 0,
        data: [
          { id: 'r1', code: 'finance-director', name: '财务总监', description: '财务部门管理者，拥有全模块读权限与数据半选', isSystem: true, userCount: 3, createdAt: '2026-08-21T11:55:00Z', permissions: [] },
          { id: 'r2', code: 'finance-manager', name: '财务经理', description: '财务一线负责人，覆盖财务/报告模块，可写数据', isSystem: true, userCount: 5, createdAt: '2026-08-22T14:08:00Z', permissions: [] },
          { id: 'r3', code: 'business-manager', name: '业务经理', description: '业务条线负责人，偏经营分析与往来模块', isSystem: true, userCount: 8, createdAt: '2026-08-23T09:28:00Z', permissions: [] },
          { id: 'r4', code: 'salesman', name: '销售员', description: '一线销售，仅看首页/经营/往来模块', isSystem: true, userCount: 24, createdAt: '2026-08-24T16:15:00Z', permissions: [] },
          { id: 'r5', code: 'readonly', name: '只读用户', description: '只读账号，仅查看公开看板与报告', isSystem: true, userCount: 12, createdAt: '2026-08-24T18:42:00Z', permissions: [] },
        ],
        message: 'ok',
      }
    } else if (url.includes('/admin/permissions') || url.includes('/permissions')) {
...
async function waitForReactReady(page) {
  ...
  await page
    ...
    .catch(() => {})
  await page.waitForSelector('text=角色管理', { timeout: 30000 }).catch(() => {})
  await page.waitForSelector('text=角色 × 模块权限矩阵', { timeout: 30000 }).catch(() => {})
  await page.waitForSelector('text=资源树勾选区', { timeout: 30000 }).catch(() => {})
  await page.waitForSelector('text=财务经理', { timeout: 30000 }).catch(() => {})
...
async function main() {
  ...
  // ─── Design 截图 ───
  {
    ...
    for (const vp of [
      ...
      await page
        .goto(`${DESIGN_BASE}/admin-roles.html`, { waitUntil: 'networkidle', timeout: 30000 })
        .catch(() => {})
  ...
  await page
    .goto(`${REACT_BASE}/admin/roles`, { waitUntil: 'domcontentloaded', timeout: 30000 })
    .catch(() => {})
