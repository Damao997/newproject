// 只截企业信息查询 / enterprise-lookup 一个页面的 React + Design 双截图（用于本地验证高保真度）
// - React: 走 http://127.0.0.1:5173/tools/enterprise-lookup（Vite dev server）
// - Design: 走 http://127.0.0.1:8001/pages/enterprise-lookup.html（design static server）
// 用法：node scripts/shoot-enterprise-lookup.mjs
// 产物：audit/screenshots/{design,react}/enterprise-lookup-{desktop,mobile}.png
...
async function installApiMock(ctx) {
  await ctx.route('**/api/v1/**', async (route) => {
    ...
    } else if (url.includes('/tools/enterprise/history')) {
      body = {
        code: 0,
        data: {
          items: [
            { id: 1, keyword: '浙江壹品慧生活服务集团', matchedName: '浙江壹品慧生活服务集团有限公司', createdAt: new Date().toISOString() },
            { id: 2, keyword: '万科物业', matchedName: '万科物业发展股份有限公司', createdAt: new Date(Date.now() - 86400000).toISOString() },
            { id: 3, keyword: '绿城服务', matchedName: '绿城物业服务集团有限公司', createdAt: new Date(Date.now() - 2 * 86400000).toISOString() },
          ],
          total: 3,
        },
        message: 'ok',
      }
    } else if (url.includes('/tools/enterprise/search')) {
      body = {
        code: 0,
        data: {
          name: '浙江壹品慧生活服务集团有限公司',
          creditCode: '91330000MA28T4XJ9P',
          legalPerson: '王志强',
          registeredCapital: '5,000 万人民币',
          establishDate: '2008-04-18',
          status: '存续',
          companyType: '有限责任公司（自然人投资或控股）',
          industry: '房地产业 / 物业管理',
          registeredAddress: '浙江省杭州市西湖区文一西路 588 号',
          businessScope: '物业管理、家政服务、房地产经纪、餐饮管理、建筑装饰工程等',
          provider: 'mock',
          fromCache: false,
...
async function waitForReactReady(page) {
  ...
  await page
    ...
    .catch(() => {})
  await page.waitForSelector('text=企业信息查询', { timeout: 30000 }).catch(() => {})
  await page.waitForSelector('text=查询企业工商与风险信息', { timeout: 30000 }).catch(() => {})
  await page.waitForSelector('text=股东信息 (前 5)', { timeout: 30000 }).catch(() => {})
  await page.waitForSelector('text=风险评估雷达', { timeout: 30000 }).catch(() => {})
  await page.waitForSelector('text=关联企业', { timeout: 30000 }).catch(() => {})
  await page.waitForSelector('text=最近变更记录', { timeout: 30000 }).catch(() => {})
...
async function waitForDesignReady(page) {
  ...
  await page.waitForSelector('text=企业信息查询', { timeout: 30000 }).catch(() => {})
  await page.waitForSelector('text=查询企业工商与风险信息', { timeout: 30000 }).catch(() => {})
  await page.waitForSelector('text=股东信息', { timeout: 30000 }).catch(() => {})
  await page.waitForSelector('text=风险评估雷达', { timeout: 30000 }).catch(() => {})
  await page.waitForSelector('text=关联企业', { timeout: 30000 }).catch(() => {})
  await page.waitForSelector('text=最近变更记录', { timeout: 30000 }).catch(() => {})
...
async function main() {
  ...
  // ─── Design 截图 ───
  {
    ...
    for (const vp of [
      ...
      await page
        .goto(`${DESIGN_BASE}/enterprise-lookup.html`, { waitUntil: 'networkidle', timeout: 30000 })
        .catch(() => {})
  ...
  await page
    .goto(`${REACT_BASE}/tools/enterprise-lookup`, { waitUntil: 'domcontentloaded', timeout: 30000 })
    .catch(() => {})
