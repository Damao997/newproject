// 只截主体覆盖 / transactions-coverage 一个页面的 React + Design 双截图（用于本地验证高保真度）
// - React: 走 http://127.0.0.1:5173/transactions/coverage（Vite dev server）
// - Design: 走 http://127.0.0.1:8001/pages/transactions-coverage.html（design static server）
// 用法：node scripts/shoot-transactions-coverage.mjs
// 产物：audit/screenshots/{design,react}/transactions-coverage-{desktop,mobile}.png
...
const SLUG = 'transactions-coverage'
...
async function installApiMock(ctx) {
  await ctx.route('**/api/v1/**', async (route) => {
    ...
    } else if (url.includes('/transactions/overview') || url.includes('/transactions/coverage')) {
      body = { code: 0, data: { kpi: {}, rows: [], summary: { coverageRate: 96.4, active: 38, empty: 6, draft: 0, missing: 1, expected: 45 } }, message: 'ok' }
...
async function waitForReactReady(page) {
  ...
  await page
    ...
    .catch(() => {})
  await page.waitForSelector('text=主体覆盖', { timeout: 30000 }).catch(() => {})
  await page.waitForSelector('text=主体 × 账龄矩阵', { timeout: 30000 }).catch(() => {})
  await page.waitForSelector('text=覆盖主体数', { timeout: 30000 }).catch(() => {})
  await page.waitForSelector('text=数据完整度', { timeout: 30000 }).catch(() => {})
...
async function main() {
  ...
  // ─── Design 截图 ───
  {
    ...
    for (const vp of [
      ...
      await page
        .goto(`${DESIGN_BASE}/transactions-coverage.html`, { waitUntil: 'networkidle', timeout: 30000 })
        .catch(() => {})
  ...
  await page
    .goto(`${REACT_BASE}/transactions/coverage`, { waitUntil: 'domcontentloaded', timeout: 30000 })
    .catch(() => {})
