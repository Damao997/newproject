// 只截看板管理 / 品类配置 / data-board-category 一个页面的 React + Design 双截图（用于本地验证高保真度）
// - React: 走 http://127.0.0.1:5173/data/board/category（Vite dev server）
// - Design: 走 http://127.0.0.1:8001/pages/data-board.html（design static server）
// 用法：node scripts/shoot-data-board-category.mjs
// 产物：audit/screenshots/{design,react}/data-board-category-{desktop,mobile}.png
...
const SLUG = 'data-board-category'
...
async function waitForReactReady(page) {
  ...
  await page
    ...
    .catch(() => {})
  await page.waitForSelector('text=品类配置', { timeout: 30000 }).catch(() => {})
  await page.waitForSelector('text=运营费用映射', { timeout: 30000 }).catch(() => {})
  await page.waitForSelector('text=主体配置', { timeout: 30000 }).catch(() => {})
  await page.waitForSelector('text=月度预算比例', { timeout: 30000 }).catch(() => {})
  await page.waitForSelector('text=产品配置', { timeout: 30000 }).catch(() => {})
...
async function main() {
...

  {
    ...
    for (const vp of [
      ...
      await page
        .goto(`${DESIGN_BASE}/data-board.html`, { waitUntil: 'networkidle', timeout: 30000 })
        .catch(() => {})
  ...
  await page
    .goto(`${REACT_BASE}/data/board/category`, { waitUntil: 'domcontentloaded', timeout: 30000 })
    .catch(() => {})
