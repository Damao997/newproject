 ...
 * 这里用路由级拦截让所有 API 走"业务正常 + 空数据"，这样：
 * 1) useKeyMetrics 走 200 分支（data 为空，仍走 MOCK 视觉层）
 * 2) isAuthenticated 保持 true，MainLayout 不再 Navigate /login
 * 3) 页面正常渲染 mock tiles / heatmap / trend / detail table
 */
...
async function main() {
  ...
  await reactPage.goto(`${REACT_BASE}/dashboard/analysis/key-metrics`, { waitUntil: 'domcontentloaded', timeout: 30000 }).catch(() => {})
  ...
  await designPage.goto(`${DESIGN_BASE}/dashboard-analysis-keymetrics.html`, { waitUntil: 'networkidle', timeout: 30000 }).catch(() => {})
  ...
  console.log('Target ≥85 KB (≈' + (designDesktopBytes * 0.67 / 1024).toFixed(1) + ' KB at 67%)')
}
