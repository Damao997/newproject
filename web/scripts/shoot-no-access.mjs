// 只截「权限不足兜底页」一个页面的 React + Design 双截图（用于本地验证高保真度）
// - React: 走 http://127.0.0.1:5173/no-access（Vite dev server）
// - Design: 走 http://127.0.0.1:8001/pages/no-access.html（design static server）
// 用法：node scripts/shoot-no-access.mjs
// 产物：audit/screenshots/{design,react}/no-access-{desktop,mobile}.png
...
async function waitForReactReady(page) {
  ...
  await page
    ...
    .catch(() => {})
  await page.waitForSelector('text=您当前没有访问该模块的权限', { timeout: 30000 }).catch(() => {})
  await page.waitForSelector('text=安全访问', { timeout: 30000 }).catch(() => {})
  await page.waitForSelector('text=联系管理员', { timeout: 30000 }).catch(() => {})
  await page.waitForSelector('text=退出登录', { timeout: 30000 }).catch(() => {})
  await page.waitForTimeout(800)
}

/**
 * 解除 MainLayout 的 h-screen/overflow-hidden 锁定，让 document.scrollHeight 反映实际内容高度。
 * no-access 走 position:fixed 拉满整屏，这里把 html/body 解除 min-height 锁即可。
 */
...
async function main() {
  ...
  // ─── Design 截图 ───
  {
    ...
    for (const vp of [
      ...
      await page
        .goto(`${DESIGN_BASE}/no-access.html`, { waitUntil: 'networkidle', timeout: 30000 })
        .catch(() => {})
  ...
  await page
    .goto(`${REACT_BASE}/no-access`, { waitUntil: 'domcontentloaded', timeout: 30000 })
    .catch(() => {})
