// 只截催收计划一个页面的 React + Design 双截图（用于本地验证高保真度）
// - React: 走 http://127.0.0.1:5173/transactions/collections/plans（Vite dev server）
// - Design: 走 http://127.0.0.1:8001/pages/transactions-collections-plans.html（design static server）
// 用法：node scripts/shoot-transactions-collections-plans.mjs
// 产物：audit/screenshots/{design,react}/transactions-collections-plans-{desktop,mobile}.png
...
const SLUG = 'transactions-collections-plans'
...
async function waitForReactReady(page) {
  ...
  await page
    ...
    .catch(() => {})
  await page.waitForSelector('text=催收计划', { timeout: 30000 }).catch(() => {})
  await page.waitForSelector('text=催收时间线', { timeout: 30000 }).catch(() => {})
...
async function main() {
  ...
  // ─── Design 截图 ───
  {
    ...
    for (const vp of [
      ...
      await page
        .goto(`${DESIGN_BASE}/transactions-collections-plans.html`, { waitUntil: 'networkidle', timeout: 30000 })
        .catch(() => {})
  ...
  await page
    .goto(`${REACT_BASE}/transactions/collections/plans`, { waitUntil: 'domcontentloaded', timeout: 30000 })
    .catch(() => {})
