...
import { useKeyMetricsProducts, useKeyMetricsProductCheck, useKeyMetricsProductMutations } from '@/hooks/api-queries'
...
import type { KeyMetricsProduct } from '@/types'
...
const EMPTY_FORM: ProductForm = { code: '', name: '', subjectKeyword: '', sortOrder: '', status: 'active' }

/**
 * 关键指标产品配置管理面板（壹品慧关键指标表「按产品分」明细）：维护产品 ↔ 收入科目名关键词的对应关系，
 * 并展示科目树变化检测结果（未覆盖科目 / 失效关键词 / 毛利镜像缺失）。
 * 独立于品类配置（品类预算达成分析），两套配置各自维护。
 */
export function ProductConfigPanel({ canCreate = false, canUpdate = false, canDelete = false }: ProductConfigPanelProps) {
  const queryClient = useQueryClient()
  const { data: products, isLoading } = useKeyMetricsProducts()
  const { data: check } = useKeyMetricsProductCheck()
  const mutations = useKeyMetricsProductMutations()
  const { confirm, element: confirmElement } = useConfirm()

  const [dialogOpen, setDialogOpen] = useState(false)
  const [editing, setEditing] = useState<KeyMetricsProduct | null>(null)
  ...
  const batchDelete = useBatchDelete<KeyMetricsProduct>({
    entityLabel: '产品配置',
  ...
  const openEdit = (row: KeyMetricsProduct) => {
    setEditing(row)
    setForm({
      code: row.code,
      name: row.name,
  ...
  const handleDelete = async (row: KeyMetricsProduct) => {
    const ok = await confirm({
      title: '删除产品配置',
      description: `将删除产品「${row.name}」，壹品慧关键指标表「按产品分」将不再展示该产品。`,
      confirmText: '删除',
      danger: true,
    })
    if (!ok) return
  ...
  const validateBatchRow = (row: ProductForm, _index: number, allRows: ProductForm[]): string | null => {
    if (!row.code.trim()) return '产品编码必填'
    if (!row.name.trim()) return '产品名称必填'
    if (!row.subjectKeyword.trim()) return '匹配关键词必填'
    if ((products ?? []).some((p) => p.code === row.code.trim())) return `编码 ${row.code.trim()} 已存在`
  ...
  const submitBatchRows = async (rows: ProductForm[]): Promise<(string | null)[]> => {
  ...
  const selectedBatchRows = (products ?? []).filter((r) => batchDelete.selectedKeys.has(r.id))

  const checkMap = new Map((check?.products ?? []).map((c) => [c.id, c]))
  ...
  // 产品列表列（含科目树变化检测结果与权限门禁行操作）
  const columns: DataTableColumn<KeyMetricsProduct>[] = useMemo(() => {
    const cols: DataTableColumn<KeyMetricsProduct>[] = [
      { key: 'name', header: '产品名称', cellClassName: 'font-medium text-foreground' },
      {
      ...
      cols.push({
        ...
        render: (row) => (
          <div className="flex justify-end gap-1">
            {canUpdate && (
              <Button variant="ghost" size="sm" className="h-7 px-2" onClick={() => openEdit(row)} title="编辑产品">
                <Pencil className="h-3.5 w-3.5" />
              </Button>
            )}
            {canDelete && (
              <Button variant="ghost" size="sm" className="h-7 px-2 text-destructive hover:text-destructive" onClick={() => handleDelete(row)} title="删除产品">
    ...
    <div className="space-y-4">
        ...
        <div className="space-y-2 rounded-lg border border-warning/30 bg-warning/5 p-3">
          {(check?.uncoveredSubjects?.length ?? 0) > 0 && (
            <div className="flex items-start gap-2 text-xs">
              ...
              <div>
                ...
                <span className="ml-2 text-muted-foreground">（新增业务线后可在此新建产品配置；分类汇总层「壹品慧收入/增值业务收入」无需配置）</span>
          ...
          {(check?.brokenKeywords?.length ?? 0) > 0 && (
            <div className="flex items-start gap-2 text-xs">
              ...
              <div>
                <span className="font-medium text-destructive">以下产品关键词在科目树中无匹配科目</span>
          ...
          {(check?.missingProfitMirror?.length ?? 0) > 0 && (
            <div className="flex items-start gap-2 text-xs">
              ...
              <div>
                <span className="font-medium text-destructive">以下产品缺少毛利镜像科目</span>
      ...
      <Dialog open={dialogOpen} onOpenChange={(o) => { if (!o && !saving) { setDialogOpen(false); setError(null) } }}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>{editing ? '编辑产品配置' : '新增产品配置'}</DialogTitle>
            <DialogDescription>
              产品名称将展示在壹品慧关键指标表「按产品分」明细中；匹配关键词对应经营科目树收入类别下的科目名（可命中多个科目并自动求和）。
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-3">
            <div className="space-y-1">
              <Label className="text-xs text-muted-foreground">产品编码（唯一，创建后不可修改）</Label>
              <Input
                value={form.code}
                onChange={(e) => setForm((f) => ({ ...f, code: e.target.value }))}
                placeholder="如 km_kitchen"
                disabled={!!editing}
                autoFocus
              />
            </div>
