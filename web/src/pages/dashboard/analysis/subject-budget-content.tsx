...
/** 主体达成率环（Ring progress）：外环=收入累计达成率，中心百分比 + 毛利/净利润达成率脚注 */
function SubjectRing({ name, code, income, profit, netProfit }: {
  name: string
  code: string
  ...
  const R = 40
  const C = 2 * Math.PI * R
  const pct = income.ytdRate === null ? 0 : Math.min(100, Math.max(0, income.ytdRate))
  const color = rateColor(income.ytdRate)
  return (
    <div className="flex flex-col items-center gap-2 rounded-card border border-border bg-card p-4 shadow-antd-1">
      ...
      <div className="relative h-[120px] w-[120px]">
        <svg viewBox="0 0 100 100" className="h-full w-full" style={{ transform: 'rotate(-90deg)' }} aria-label={`${name} 收入累计达成率`}>
        ...
        <div className="pointer-events-none absolute inset-0 flex flex-col items-center justify-center">
          <span className={cn('font-num text-lg font-semibold', income.ytdRate === null && 'text-muted-foreground')}>
            {income.ytdRate === null ? '–' : `${income.ytdRate.toFixed(1)}%`}
          </span>
          <span className="text-[10px] text-muted-foreground">收入达成</span>
        </div>
      </div>
      <div className="grid w-full grid-cols-2 gap-1 text-center text-[11px] text-muted-foreground">
        <span>
          毛利 <span className="font-num font-medium text-foreground">{profit.ytdRate === null ? '–' : `${profit.ytdRate.toFixed(0)}%`}</span>
        </span>
        <span>
          净利 <span className="font-num font-medium text-foreground">{netProfit ? (netProfit.ytdRate === null ? '–' : `${netProfit.ytdRate.toFixed(0)}%`) : '–'}</span>
        </span>
      </div>
 ...
 * - 主体达成率 Ring 网格（每主体一环：收入累计达成率，毛利/净利润达成率脚注）；
    ...
    <div className="animate-fade-in space-y-4">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {(() => {
          const tiles: { label: string; value: string; unit: string; foot: string; accent: string }[] = [
            { label: '主体数', value: String(rows.length), unit: '家', foot: mode === 'summary' ? '汇总主体成员明细' : '单体公司口径', accent: 'before:bg-info-500' },
          ...
          return (
            <>
              ...
              <div className="relative flex flex-col gap-2 overflow-hidden rounded-card border border-border bg-card p-5 shadow-antd-1 before:absolute before:bottom-0 before:left-0 before:top-0 before:w-[3px] before:bg-orange-500 before:content-['']">
                ...
                <span className="text-xs text-muted-foreground">Σ累计实际 ÷ Σ年度预算</span>
              </div>
            </>
          )
        })()}
      ...
      <Card className="border border-border shadow-antd-1">
        <CardContent className="p-5">
          <div className="mb-3 flex items-baseline gap-2">
            <h3 className="text-base font-semibold text-foreground">主体收入达成率</h3>
            <span className="text-xs text-muted-foreground">环色：≥75 绿 / 60-75 橙 / &lt;60 红 · 无预算灰</span>
          </div>
      ...
      <Card className="border border-border shadow-antd-1">
        <CardContent className="px-5 py-5">
          <div className="mb-3 flex items-baseline gap-2">
            <h3 className="text-base font-semibold text-foreground">主体预算明细</h3>
            <span className="text-xs text-muted-foreground">收入 / 毛利 / 净利润 · 月度/累计口径 · 单位：万元</span>
          </div>
