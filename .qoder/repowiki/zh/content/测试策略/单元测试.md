...
- [InventoryService.test.ts](file://server/src/services/InventoryService.test.ts)
- [DashboardService.test.ts](file://server/src/services/DashboardService.test.ts)
...
## 更新摘要
**变更内容**   
- 新增库存服务测试模块（InventoryService.test.ts，141行）
- 新增仪表板服务测试模块（DashboardService.test.ts，143行）  
- 新增KPI卡片组件测试（kpi-card.test.tsx，70行）
- 扩展前后端测试覆盖范围，完善服务层和UI组件测试体系
...
## 简介
本文件面向pj3项目的单元测试实践，聚焦于基于Vitest的前后端测试体系。内容涵盖：
- 测试环境设置与初始化
- 断言库选择与使用策略
- 模拟对象（Mock）创建与管理
- 工具函数、业务逻辑与纯函数的测试方法
...
**更新** 本次更新重点介绍了新增的库存服务、仪表板服务和KPI卡片的测试实现，展示了前后端测试的最佳实践。
...
### KPI卡片组件测试（kpi-card.test.tsx）
新增的KPI卡片组件测试展示了前端UI组件测试的最佳实践，包含70行高质量测试用例。

- 测试目标：验证KPI卡片的数值显示、趋势指示、颜色映射等核心功能
- 测试场景：
  - 正常数据渲染与格式化
  - 负值与零值的特殊处理
  - 趋势方向的视觉反馈
  - 空数据与异常数据的容错处理
- 技术要点：
  - 使用React Testing Library进行组件渲染测试
  - Mock外部依赖和数据接口
  - 验证用户交互和状态更新
...
新增的库存服务测试模块包含141行测试用例，全面覆盖库存管理的核心业务逻辑。

- 测试目标：验证库存服务的CRUD操作、数据验证、业务规则校验等功能
- 测试场景：
  - 库存数据的增删改查操作
  - 库存数量验证与边界条件处理
  - 库存分类与标签管理
...
### 仪表板服务测试（DashboardService.test.ts）
新增的仪表板服务测试模块包含143行测试用例，确保仪表板数据的准确性和可靠性。

- 测试目标：验证仪表板数据聚合、指标计算、数据可视化准备等功能
- 测试场景：
  - 多维度数据聚合与统计
  - 时间序列数据的处理
  - 指标计算公式的正确性
...
graph TB
subgraph "前端测试"
A["kpi-card.test.tsx<br/>KPI卡片组件测试"]
B["data-table.test.tsx<br/>数据表格测试"]
C["require-permission.test.tsx<br/>权限守卫测试"]
D["usePermission.test.tsx<br/>权限Hook测试"]
end
subgraph "后端测试"
E["InventoryService.test.ts<br/>库存服务测试"]
F["DashboardService.test.ts<br/>仪表板服务测试"]
end
G["vitest.config.ts<br/>测试配置"] --> A
G --> B
G --> C
G --> D
G --> E
G --> F
H["setup.ts<br/>测试初始化"] --> A
H --> B
H --> C
H --> D
```
...
Vitest --> TestsD["charts/__tests__/kpi-card.test.tsx"]
Vitest --> TestsE["services/InventoryService.test.ts"]
Vitest --> TestsF["services/DashboardService.test.ts"]
...
- **更新** 新增的服务层测试需要特别注意数据库Mock的性能影响，建议使用内存数据库或高效的Mock策略。
...
- **更新** 服务层测试常见问题：
  - 数据库连接问题：确保测试环境正确配置数据库连接
  - 异步操作超时：适当增加测试超时时间或优化Mock响应
  - 内存泄漏：检查测试后是否正确清理全局状态和资源
...
## 结论
通过统一的Vitest配置与就近组织的__tests__目录，pj3项目实现了清晰、可维护的测试体系。配合完善的初始化脚本与Mock策略，能够高效覆盖工具函数、业务逻辑与UI组件，并在CI中建立稳定的质量门禁。

**更新** 本次新增的库存服务、仪表板服务和KPI卡片测试进一步完善了前后端测试覆盖，展现了服务层测试和UI组件测试的最佳实践。建议在后续迭代中持续优化覆盖率阈值与测试分层，进一步提升整体质量与开发效率。
...
### 服务层测试最佳实践
**新增** 基于新增的库存服务和仪表板服务测试，总结以下最佳实践：
- 数据库Mock：使用内存数据库或高效的Mock策略，避免真实数据库依赖
- 异步操作：正确处理Promise和异步函数，确保测试的确定性
- 业务规则：验证复杂的业务逻辑和数据验证规则
- 性能测试：监控内存使用和响应时间，确保大规模数据处理能力
- 错误处理：测试各种异常场景和错误恢复机制
...
### UI组件测试最佳实践
**新增** 基于新增的KPI卡片组件测试，总结以下最佳实践：
- 组件渲染：验证组件在不同数据状态下的渲染结果
- 用户交互：测试用户操作和状态更新
- 样式验证：确保视觉反馈的正确性（颜色、图标、布局）
- 无障碍性：验证组件的可访问性和键盘导航
- 性能优化：避免不必要的重新渲染和内存泄漏
