...
## 更新摘要
**所做更改**   
- 更新了文件上传大小限制从 50MB 到 200MB 的相关章节
- 修正了 API 错误消息中的文件大小限制描述
- 更新了文件验证流程说明以反映新的限制
- 增强了文件上传安全部分的详细说明
...
- 富文本内容与用户输入数据处理策略
- 文件上传安全：类型校验、大小限制（已更新至 200MB）、恶意文件检测、存储路径安全
...
- lib/file-validation.ts：文件上传前的类型、大小等校验（已更新支持 200MB 限制）
...
F["文件上传校验<br/>lib/file-validation.ts<br/>最大 200MB"]
...
participant V as "前端校验<br/>file-validation.ts<br/>最大 200MB"
...
**已更新** 文件大小限制已从 50MB 提升至 200MB，以支持 ERP 大账龄报表等业务需求（可达 100MB+）

```mermaid
flowchart TD
Select(["选择文件"]) --> Validate["执行 file-validation.ts<br/>最大 200MB"]
Validate --> TypeOK{"类型合法?"}
TypeOK --> |否| RejectType["拒绝并提示类型不合法"]
TypeOK --> |是| SizeOK{"大小合规?<br/>(≤ 200MB)"}
SizeOK --> |否| RejectSize["拒绝并提示超出大小限制<br/>超过 200MB"]
...
**已更新** 文件大小限制错误消息已更新为 200MB，与前端校验保持一致

```mermaid
sequenceDiagram
participant Page as "页面"
participant API as "api.ts<br/>文件大小限制 : 200MB"
...
API["api.ts<br/>文件大小限制: 200MB"] --> Sanitize
API --> FileVal["file-validation.ts<br/>最大 200MB"]
...
- 上传节流：大文件分片与进度反馈，提升用户体验（支持 200MB 大文件）
- 规则精简：白名单尽量精确，避免过度解析导致性能下降

## 故障排查指南
- 常见问题
  - 富文本渲染异常：确认是否经过 sanitize 且白名单配置正确
  - 上传失败：检查 MIME 与扩展名一致性、文件大小限制（已更新为 200MB）
...
**已更新** 文件大小限制故障排查：确认前端校验和后端错误处理都使用 200MB 限制
...
## 结论
通过统一的 sanitize 工具、严格的文件上传校验（已支持 200MB 大文件）、完善的 API 参数校验以及安全的渲染策略，pj3 项目在输入验证与输出编码方面形成了闭环。建议在后续迭代中持续完善白名单策略、增强恶意文件检测能力，并将校验规则集中化管理，以提升整体安全性与可维护性。
...
- 最佳实践清单
  ...
  - 输出时遵循"晚输出、强编码"原则
  - 富文本采用白名单与深度限制
  - 文件上传前后端双重校验，启用内容探测（支持 200MB 限制）
  - 统一错误码与用户提示，便于审计与排障
- 参考规范
