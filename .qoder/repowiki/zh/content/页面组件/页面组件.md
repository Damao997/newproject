...
- [web/src/components/layout/nav-items.ts](file://web/src/components/layout/nav-items.ts)
...
- [web/src/components/dimension/subject-budget-panel.tsx](file://web/src/components/dimension/subject-budget-panel.tsx)
</cite>

## 更新摘要
**变更内容**   
- 新增"看板管理"二级菜单，包含品类配置和主体配置功能
- 优化侧边栏导航结构，支持三级菜单层级
- 改进URL标准化机制，自动处理tab和sub参数
- 增强数据浏览功能的行展开保护（500行限制）
- 统一分类和科目配置界面的用户体验
...
**最新更新**：侧边栏导航已重新组织，新增"看板管理"二级菜单，支持品类配置和主体配置功能，同时改进了URL标准化和行展开保护机制。
...
C --> D["侧边栏 sidebar.tsx"]
D --> E["导航项 nav-items.ts"]
E --> F["看板管理<br/>data?tab=board"]
F --> G["品类配置<br/>product-category-panel.tsx"]
F --> H["主体配置<br/>subject-budget-panel.tsx"]
C --> I["页面容器 page-container.tsx"]
I --> J["业务页面<br/>dashboard / admin / reports / inventory / transactions / data / indicators / login"]
J --> K["权限装饰 require-permission.tsx"]
J --> L["图表与表格组件<br/>charts / data-table"]
J --> M["UI基础组件<br/>ui/*"]
J --> N["全局状态 stores/authStore.ts"]
J --> O["权限钩子 hooks/usePermission.ts"]
J --> P["认证钩子 hooks/useAuth.ts"]
J --> Q["API封装 lib/api.ts"]
J --> R["常量 lib/constants.ts"]
J --> S["类型 types/index.ts"]
```
...
- 布局组件
  ...
  - sidebar：响应式侧边栏，支持桌面端折叠面板和移动端抽屉模式，集成权限过滤。
...
- 业务页面
  ...
  - data：数据看板与数据源配置入口，现包含看板管理功能。
  - indicators：指标管理与自定义指标维护。
  - login：登录页，处理认证流程与错误提示。
- 维度配置组件
  - product-category-panel：品类配置管理，支持品类与收入科目关键词匹配。
  - subject-budget-panel：主体预算配置管理，支持公司主体展示配置。
- 通用能力
...
- 布局壳层：main-layout与sidebar负责页面骨架与导航，page-container提供统一页面容器。
...
participant P as "页面(如 data/board)"
...
- sidebar
  - 职责：响应式侧边栏导航，支持桌面端折叠面板和移动端抽屉模式。
  - 新功能：集成权限过滤，仅显示当前角色有view权限的模块入口。
  - 交互特性：支持手风琴展开、固定面板弹出、滚动关闭等高级交互。
- page-container
...
- 导航级权限
  - sidebar组件根据用户权限动态过滤导航项，仅显示有view权限的模块。
...
#### 看板管理概述
看板管理是数据管理模块的新增二级菜单，提供品类配置和主体配置两大功能，用于支持看板预算达成分析。

#### 品类配置（Product Category Panel）
- 功能特点
  - 维护品类与收入科目树收入类别的对应关系
  - 通过匹配关键词自动匹配收入科目（名称包含关键词，可命中多个并求和）
  - 毛利数据按镜像科目（"XX收入"→"XX毛利"）自动配对
  - 实时检测未覆盖科目、失效关键词、毛利镜像缺失等问题
- 操作流程
  - 新增品类：填写编码、名称、匹配关键词、排序和状态
  - 编辑品类：修改现有配置信息
  - 删除品类：确认删除后不再参与看板分析
  - 刷新检测：重新扫描科目树变化并生成检测报告

#### 主体配置（Subject Budget Panel）
- 功能特点
  - 维护看板主体预算达成分析展示的主体（排序/启停）
  - 仅配置且启用（active）的主体会在看板中展示
  - 自动检测公司表新增但未配置的主体
  - 支持单体公司和汇总主体的区分管理
- 操作流程
  - 新增主体：从未配置的公司/汇总主体中选择
  - 编辑主体：调整排序和状态
  - 移除主体：从看板展示列表中移除
  - 刷新检测：扫描公司表变化并提示未配置主体

#### URL标准化机制
- tab参数标准化
  - dimensions子标签：operating、static、company、summary
  - board子标签：category、subject
- sub参数标准化
  - 非法或缺失的sub参数自动回退到默认值
  - 冗余的manage标签自动清除，保证叶子节点精确高亮
- 历史记录管理
  - 使用replace模式避免产生多余历史记录
  - 路由切换时自动清理无效参数
...
- 组件耦合
  ...
  - sidebar组件依赖nav-items.ts中的导航配置，实现解耦的菜单管理。
- 外部依赖
...
- **更新**：行展开保护机制防止内存溢出，500行限制确保大数据集下的性能稳定。
...
- 常见问题
  ...
  - 导航不显示：检查用户权限配置与nav-items.ts中的resource字段匹配。
  - 看板配置无效：确认品类关键词与科目树名称匹配，检查镜像科目是否存在。
- 定位步骤
  - 打开浏览器控制台与网络面板，观察请求与错误堆栈。
  - 在权限组件处打印当前权限集合，比对期望权限。
  - 在API拦截器中打印请求头与响应体，验证鉴权与错误码。
  - 检查URL参数标准化逻辑，确认tab和sub参数正确解析。
...
## 结论
pj3页面组件体系以"布局壳 + 业务页 + 能力层"的分层架构为核心，配合声明式权限与统一数据访问，实现了高内聚、低耦合的可扩展前端工程。通过路由守卫、组件级与数据级权限的多层次防护，以及懒加载、代码分割与缓存等性能优化手段，系统在安全性与用户体验之间取得良好平衡。

**最新更新**：侧边栏导航重新组织后，新增的看板管理功能为数据分析提供了更强大的配置能力。品类配置和主体配置模块的引入，使得看板预算达成分析更加灵活和准确。URL标准化机制和行展开保护机制的完善，进一步提升了系统的稳定性和用户体验。遵循本文档提供的标准化流程与最佳实践，可高效扩展新页面并保持整体一致性。
...
- 导航配置
  - 在nav-items.ts中添加新的导航项，配置path、label、icon和resource字段。
- 测试与自测
...
- 对导出与耗时操作提供进度反馈与取消能力。
- 严格区分只读与写权限，最小化授予原则。
- **更新**：合理设置行展开限制，防止内存溢出影响性能。
- **更新**：实现URL参数标准化，确保导航高亮与页面状态一致性。
- **更新**：在导航配置中正确设置resource权限标识，确保权限过滤生效。
