...
## 更新摘要
**变更内容**   
- **新增金额单位选择功能**：在导入面板中增加了数值单位选择控件，支持"元"和"万元"两种单位
- **四位数精度处理**：系统内部统一使用万元存储，当选择"元"时自动进行÷10000转换，保留四位小数精度
- **前后端协同处理**：前端传递valueUnit参数，后端根据单位进行相应的数值转换处理
- **用户界面增强**：在模板类型选择旁边新增了数值单位下拉选择器，提供直观的单位切换体验
...
经过本次金额单位选择功能的增强，数据管理页面现在能够灵活处理不同单位的数值输入，显著提升数据导入的准确性和用户体验。
...
**更新** 新增了完整的金额单位选择功能，通过前端选择器和后端转换器的协同工作，实现了灵活的数值单位处理能力。
...
**更新** 通过新增的金额单位选择功能，数据管理页面现在能够灵活处理不同单位的数值输入，提供更好的数据导入体验。
...
## 架构总览
下图展示了重构后的数据管理页面架构，包括持久化状态管理、专用导入面板、批量激活系统和金额单位选择功能的端到端调用链路与职责划分：
...
**更新** 专用的导入面板组件现在集成了完整的批量激活功能和金额单位选择功能，提供了更强大的数据管理能力。
...
**更新** 新增了金额单位选择功能的依赖关系，形成了更加完整的数据管理架构。
...
## 结论
通过"专用导入面板 + 四标签页架构 + 专业表格容器 + 内部实现 + 基础表格 + 批量激活系统 + 持久化状态管理 + 金额单位选择功能"的分层设计，数据管理页面实现了高内聚、低耦合的可复用方案。**新增的持久化状态管理系统**通过 zustand + persist 模式实现了公司多选、期间筛选、科目类型选择和行展开状态的自动保存和恢复，显著提升了用户体验。**新增的批量激活功能**包括冲突检测引擎、多选择复选框、进度跟踪和失败详情显示，进一步增强了数据操作的效率和安全性。**新增的金额单位选择功能**允许用户灵活选择"元"或"万元"单位，系统自动处理数值转换，提升了数据导入的准确性和易用性。专用导入面板组件的引入使得数据导入功能更加模块化和可维护，同时增强的数据质量概览功能提供了更好的用户体验。配合类型定义与 API 封装，能够快速搭建具备筛选、搜索、分页、批量、导出、树形等多能力的通用数据界面。经过本次金额单位选择功能增强，界面结构更加清晰，功能模块化程度更高，性能表现更佳，可维护性显著提升。后续可按扩展指南按需定制列渲染、操作按钮与数据源适配。
