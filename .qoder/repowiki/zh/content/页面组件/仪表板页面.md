...
## 更新摘要
**变更内容**   
- 基于Applied Changes更新：新增`resolveDashboardCompany`函数，改进公司范围处理逻辑，支持降级模式和空状态处理
- 增强了仪表板的权限处理和主体选择机制，支持越权访问时的优雅降级
- 优化了空数据状态的展示体验，提供更友好的用户提示
- 完善了前后端公司范围解析的一致性，确保数据权限的正确性
...
**更新** 基于最新的架构重构，仪表板页面已从单体页面转变为模块化卡片系统，新增了品类预算达成分析和主体预算达成分析两个重要功能模块，显著提升了预算管理的可视化能力。同时，公司范围处理逻辑得到了大幅改进，提供了更健壮的权限控制和用户体验。
...
**更新** 架构重构后，仪表板采用了更加模块化的设计，新增了品类预算达成分析和主体预算达成分析两个重要的预算管理功能模块，每个业务功能都有独立的卡片组件，提高了代码的可维护性和复用性。同时，公司范围处理逻辑得到了大幅改进，提供了更健壮的权限控制。
...
**更新** 主页面现在主要负责协调各个模块化卡片组件，包括新增的品类预算达成卡和主体预算达成卡，不再直接处理具体的业务逻辑，大大简化了页面复杂度。同时增强了对降级模式的处理，确保在权限异常时仍能提供可用的数据视图。
...
### 降级模式支持
- 前端处理：通过`data?.degraded`标志识别降级状态，自动同步实际生效的主体
- 用户体验：防止下拉框出现空白选项，确保用户始终看到有效的主体选择
- 状态同步：当前主体显示名使用后端返回的实际生效主体名称
- 错误处理：区分真正的错误（如网络问题）和降级情况（权限不足）
...
**更新** 新增的公司范围解析服务和降级模式支持，使仪表板在处理权限异常时更加健壮，用户体验得到显著提升。空状态处理机制也为用户提供了更清晰的反馈。
...
**更新** 模块化架构使得各个组件的职责更加明确，特别是新增的预算分析组件和公司范围解析服务，数据流向也更加清晰，便于单元测试和维护。
...
**更新** 模块化架构带来了更好的性能表现，特别是新增的预算分析组件和公司范围解析服务可以独立优化和管理自己的渲染逻辑，减少了不必要的重渲染。降级模式的支持也提升了用户体验，避免了因权限问题导致的长时间等待。
...
**更新** 新增了针对公司范围解析和降级模式的故障排查指导，特别关注权限配置、降级逻辑和空状态处理的问题。

## 结论
仪表板页面通过模块化卡片系统的架构重构，实现了更好的代码组织、更高的可维护性和更强的扩展性。新增的品类预算达成分析和主体预算达成分析功能模块，为预算管理提供了全面的数据可视化支持。同时，公司范围处理逻辑的大幅改进，通过`resolveDashboardCompany`函数实现了智能的权限控制和优雅降级，显著提升了系统的健壮性和用户体验。每个业务功能都有独立的卡片组件，职责清晰、边界明确。结合响应式布局与主题定制能力，可在多设备上保持一致体验。基于最新的架构改进，仪表板现在支持更好的组件复用、更清晰的数据流向、更高效的性能表现和更健壮的权限处理。建议在后续迭代中持续完善错误边界、可观测性与性能监控，以提升稳定性与可维护性。

**更新** 新增的公司范围解析服务和降级模式支持，使仪表板在处理权限异常时更加健壮，为用户提供了更好的使用体验。空状态处理机制也为用户提供了更清晰的反馈，避免了困惑和误解。
...
**更新** 新增了公司范围解析功能的开发指南，便于扩展仪表板的权限处理能力，特别是`resolveDashboardCompany`函数的实现和降级逻辑的配置方法。

**章节来源**
- [web/src/pages/dashboard/index.tsx:20-209](file://web/src/pages/dashboard/index.tsx#L20-L209)
