...
- [company-select.tsx](file://web/src/components/filters/company-select.tsx)
...
## 更新摘要
**变更内容**   
- 新增公司选择过滤器组件章节，详细介绍 CompanySelect 和 CompanyMultiSelect 组件
- 更新筛选器集成部分，说明标准化筛选界面的实现模式
- 扩展使用示例，展示公司选择器在数据表格中的实际应用
- 完善架构总览图，包含新的过滤器组件
...
**更新** 新增公司选择过滤器组件的详细说明，该组件为应用提供了标准化的公司筛选界面，消除了重复实现。

## 项目结构
数据表格相关代码位于 web/src/components/data-table 目录下，包含基础表格、高级表格、内部编排器与分页组件。公司选择过滤器组件位于 web/src/components/filters 目录下，为整个应用提供统一的公司筛选功能。
...
subgraph "过滤器模块"
CS["company-select.tsx<br/>公司选择器"]
end
PDT --> PTI
PTI --> DT
PTI --> PAG
...
- **company-select**：公司选择过滤器组件，提供单选和多选两种模式，内置"全部公司"选项，名称跟随全局显示简称设置。
...
## 架构总览
整体采用"组合式"架构：pro-data-table 作为门面，内部通过 pro-table-inner 编排数据流与状态，最终交由 data-table 进行渲染；pagination 作为独立可复用控件被上层集成。公司选择过滤器组件作为独立的筛选器，可与数据表格组件配合使用，提供标准化的公司筛选功能。
...
participant CS as "company-select"
U->>P : 配置/事件回调
U->>CS : 公司筛选操作
CS-->>P : 筛选状态变更
...
### company-select 公司选择过滤器
- 职责
  - 提供标准化的公司筛选界面，支持单选和多选模式。
  - 内置"全部公司"选项，简化筛选逻辑。
  - 名称显示跟随全局"显示简称"设置，保持一致性。
- 关键特性
  - **CompanySelect**：单选模式，支持 entitiesOnly 参数仅显示单体公司。
  - **CompanyMultiSelect**：多选模式，支持全选/清空操作，智能显示选中数量。
  - 自动获取公司列表数据，无需手动管理数据源。
  - 响应式设计，适配不同屏幕尺寸。
- 设计要点
  - 完全受控组件，通过 value 和 onChange 控制状态。
  - 内置数据获取逻辑，减少页面组件复杂度。
  - 支持自定义样式类名，便于主题定制。
- 典型用法
  - 简单筛选：`<CompanySelect value={selected} onChange={setSelected} />`
  - 多选筛选：`<CompanyMultiSelect value={selected} onChange={setSelected} />`
  - 仅单体公司：`<CompanySelect entitiesOnly value={selected} onChange={setSelected} />`
...
- 组件内聚
  ...
  - company-select 组件独立于数据表格，可单独使用或与其他组件配合。
- 外部依赖
  - types/index.ts 提供列定义、筛选器、分页、选择等类型约束，贯穿各组件。
  - company-select 依赖 useCompanies hook 获取公司数据，useCompanyDisplay hook 处理名称显示。
- 潜在循环
  - 通过单向数据流与事件回调避免循环依赖。
...
Types --> CS["company-select.tsx"]
PDT --> PTI
PTI --> DT
PTI --> PAG["pagination.tsx"]
CS --> Hooks["api-queries hooks"]
CS --> Display["useCompanyDisplay"]
```
...
- **公司选择器优化**
  - 使用 useMemo 缓存公司列表数据，避免重复计算。
  - 公司名称映射使用 Map 结构，提高查找效率。
  - 多选模式下智能显示选中项，避免过长文本影响布局。
...
- 常见问题
  ...
  - **公司选择器问题**：确认 useCompanies hook 数据正常加载，displayNameMap 正确映射。
- 调试建议
  - 打印 pro-table-inner 的输入/输出快照，定位数据流断点。
  - 使用浏览器性能面板观察重渲染热点，定位重型渲染列。
  - 针对边界用例编写单测，覆盖空数据、超长文本、特殊字符等。
  - 检查公司选择器的 entitiesOnly 参数是否正确过滤公司类型。
- 参考测试
...
## 结论
data-table 与 pro-data-table 形成清晰的分层：前者专注渲染与基础交互，后者封装高级能力并通过 pro-table-inner 统一管理数据流与状态。新增的公司选择过滤器组件为应用提供了标准化的筛选界面，消除了重复实现，提升了代码复用性和维护性。该架构具备良好的扩展性与可测试性，适合在复杂业务场景中复用。结合虚拟滚动、懒加载与合理的状态缓存策略，可在大数据量下保持流畅体验。
...
- 筛选
  ...
  - **公司筛选**：CompanySelect 支持单选，CompanyMultiSelect 支持多选，entitiesOnly 参数控制公司类型。
- 选择
  - 单选/多选、全选、选择回调。
- 编辑
  - 行级/单元格级编辑、保存回调、校验规则。
...
- **公司选择器使用**
  - 单选模式：[inventory-pie-card.tsx](file://web/src/pages/dashboard/inventory-pie-card.tsx)
  - 多选模式：[data/index.tsx](file://web/src/pages/data/index.tsx), [inventory/index.tsx](file://web/src/pages/inventory/index.tsx)
  - 组合使用：[transactions/index.tsx](file://web/src/pages/transactions/index.tsx)

### 公司选择器集成示例
以下是在数据表格页面中集成公司选择器的典型模式：
...
**章节来源**
- [company-select.tsx](file://web/src/components/filters/company-select.tsx)
- [inventory-pie-card.tsx](file://web/src/pages/dashboard/inventory-pie-card.tsx)
